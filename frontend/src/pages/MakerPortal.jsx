      ...
      <AnimatePresence mode="wait">
        ...
        {step === 0 && (
          <motion.div key="step0" variants={slideVariants} initial="initial" animate="enter" exit="exit" className="w-full">
            <div className="mb-10">
              <h2 className="text-[32px] leading-tight font-extrabold tracking-tight text-primary mb-2">
            ...
            {isExtracting ? (
              ...
              <div className="bg-panel-strong border border-default rounded-2xl overflow-hidden mb-14 shadow-lg">
                <div className="p-8">
                  <form onSubmit={handleSubmit}>
                    {inputMode === 'url' ? (
                      <>
                        <label className="block text-primary font-semibold mb-3 text-[15px] uppercase tracking-wider text-xs text-muted">YOUTUBE OR GITHUB URL</label>
                        <div className="bg-white dark:bg-surface-strong border border-default rounded-xl p-1 mb-6 flex items-center transition-all focus-within:border-[#3ecf8e]/40 focus-within:ring-1 focus-within:ring-[#3ecf8e]/40">
                          <input
                            type="url"
                            value={url}
                            onChange={(e) => setUrl(e.target.value)}
                            placeholder="https://youtube.com/watch?v=..."
                            className="w-full bg-transparent px-4 py-3 text-primary placeholder-neutral-600 focus:outline-none font-mono text-sm"
                      ...
                      <>
                        <label className="block text-primary font-semibold mb-3 text-[15px] uppercase tracking-wider text-xs text-muted">SITUATION INPUT</label>
                        <div className="bg-white dark:bg-surface-strong border border-default rounded-xl p-1 mb-6 flex flex-col transition-all focus-within:border-[#3ecf8e]/40 focus-within:ring-1 focus-within:ring-[#3ecf8e]/40">
                          <textarea
                            value={prompt}
                            onChange={(e) => setPrompt(e.target.value)}
                            rows={5}
                            className="w-full bg-transparent p-4 text-primary placeholder-neutral-600 focus:outline-none text-sm leading-relaxed resize-none"
                            required={inputMode === 'text'}
                          />
                          <div className="px-4 pb-3 flex justify-between items-center mt-2 border-t border-default/50 pt-3">
                            <span className="text-muted text-xs">{prompt.length} characters</span>
                          </div>
                        </div>
                      </>
                    )}
            ...
            <div>
              <h3 className="text-[11px] font-bold text-accent tracking-[0.2em] uppercase mb-5">Recent Projects</h3>
              <div className="space-y-4">
                {projects.map((project, idx) => (
                  <motion.div 
                    ...
                    className="bg-[#f8f9fa] dark:bg-surface-soft border border-default rounded-xl p-5 cursor-pointer transition-colors duration-200 flex items-center justify-between group shadow-sm"
                  >
                    <div className="flex items-center gap-5">
                      {project.thumbnail_url ? (
                        ...
                        <div className="w-24 h-16 rounded-lg shrink-0 bg-soft border border-default flex items-center justify-center">
                          <FiHelpCircle className="text-neutral-600 w-6 h-6" />
                        </div>
                      )}
                      <div>
                        <h4 className="text-primary font-bold text-[17px] mb-1.5 group-hover:text-neutral-200 transition-colors line-clamp-1">{project.title}</h4>
                        <div className="text-sm text-muted font-medium flex items-center">
                          {project.author_name && <span className="mr-3 text-accent/80 truncate max-w-[120px]">{project.author_name}</span>}
                    ...
                    <div className="flex items-center text-accent font-bold text-[15px]">
            ...
            <div className="flex items-start justify-between mb-8">
              <div className="flex-1 mr-8">
                <div className="flex items-center group mb-1 border-b border-transparent hover:border-neutral-300 dark:border-neutral-700 focus-within:border-[#3ecf8e] pb-1 transition-colors w-fit">
                  <FiEdit3 className="text-muted mr-3 opacity-0 group-hover:opacity-100 transition-opacity" />
                  <input 
                    type="text" 
                    value={currentProject.title} 
                    onChange={updateTitle}
                    className="text-[32px] font-extrabold text-primary tracking-tight bg-transparent outline-none w-full min-w-[300px]"
                    placeholder="Name your build..."
                  />
                </div>
                <p className="text-muted text-sm mt-1">Check off items as you buy them, and track your quantities.</p>
              </div>
              <button onClick={() => setStep(2)} className="bg-accent hover:bg-accent-strong text-black font-bold rounded-lg px-6 py-2.5 active:scale-[0.98] transition-all flex items-center shadow-[0_0_15px_rgba(36,180,126,0.2)]">
                SEARCH SUPPLIERS <FiChevronRight className="ml-2" />
              </button>
            </div>

            <div className="bg-[#f8f9fa] dark:bg-surface-soft border border-default rounded-xl overflow-hidden shadow-lg p-6">
              <div className="grid grid-cols-12 text-primary font-bold text-sm mb-4 px-2 border-b border-default pb-4">
              ...
              <div className="space-y-2">
                {currentProject.components?.map((item, idx) => (
                  <div key={idx} className={`grid grid-cols-12 items-center py-4 px-2 hover:bg-white/[0.02] transition-colors border-b border-default/50 ${item.isBought ? 'opacity-50' : ''}`}>
                    ...
                    <div className={`col-span-3 text-sm ${item.isBought ? 'text-neutral-600 line-through' : 'text-muted'}`}>{item.notes || 'Standard Spec'}</div>
                    <div className="col-span-2 flex flex-col items-center justify-center">
                      <div className="flex items-center space-x-2 bg-surface-strong border border-default rounded-lg px-2 py-1">
                        <button onClick={() => updateQty(idx, -1)} className="text-muted hover:text-primary px-2">-</button>
                        <span className="text-primary font-mono w-4 text-center">{item.qty || 1}</span>
                        <button onClick={() => updateQty(idx, 1)} className="text-muted hover:text-primary px-2">+</button>
                      </div>
                      <span className="text-[10px] text-accent mt-1 font-semibold bg-accent/10 px-1.5 py-0.5 rounded border border-[#24b47e]/20">Orig: 1</span>
                    </div>
                    <div className="col-span-2 flex justify-center">
                      <span className="bg-accent/10 text-accent text-xs font-bold px-2 py-1 rounded border border-[#24b47e]/20">High</span>
                    </div>
                    <div className="col-span-1 text-right">
                      <button onClick={() => deleteItem(idx)} className="text-muted hover:text-red-400 transition-colors text-sm flex items-center justify-end w-full">
              ...
              <div className="mt-8 flex items-center space-x-4">
                <div className="flex-1 bg-white dark:bg-surface-strong border border-default rounded-lg p-3">
                  <input type="text" value={customName} onChange={(e) => setCustomName(e.target.value)} placeholder="New Custom Item..." className="bg-transparent text-primary w-full outline-none text-sm" />
                </div>
                <div className="flex-1 bg-white dark:bg-surface-strong border border-default rounded-lg p-3">
                  <input type="text" value={customSpec} onChange={(e) => setCustomSpec(e.target.value)} placeholder="Specify attributes..." className="bg-transparent text-primary w-full outline-none text-sm" />
                </div>
                <button onClick={addCustomItem} className="border border-[#24b47e] text-accent hover:bg-accent/10 font-bold rounded-lg px-5 py-3 transition-all flex items-center text-sm uppercase tracking-wider shrink-0">
                  <FiPlus className="mr-2" /> Add Custom Item
                </button>
              </div>
            </div>
            
            <button onClick={() => setStep(0)} className="mt-8 text-muted hover:text-primary transition-colors flex items-center text-sm font-semibold">
        ...
        {step === 2 && currentProject && (
          <motion.div key="step2" variants={slideVariants} initial="initial" animate="enter" exit="exit" className="w-full">
            <div className="flex items-start justify-between mb-8">
              <div>
                <h1 className="text-[32px] font-extrabold text-primary tracking-tight">Sourced Parts</h1>
                <p className="text-muted text-sm mt-1">I found these active components in supplier warehouse stocks. Pick the best matching version.</p>
              </div>
              <button onClick={() => setStep(3)} className="bg-accent hover:bg-accent-strong text-black font-bold rounded-lg px-6 py-2.5 active:scale-[0.98] transition-all flex items-center shadow-[0_0_15px_rgba(36,180,126,0.2)]">
            ...
            <div className="space-y-8">
              {currentProject.components?.map((item, idx) => (
                <div key={idx} className="bg-[#f8f9fa] dark:bg-surface-soft border border-default rounded-2xl p-6">
                  <div className="flex justify-between items-center mb-5 border-b border-default pb-4">
                    <div className="flex items-center">
                      <h3 className="text-primary font-bold text-[17px] mr-3">{item.local}</h3>
                      <span className="bg-accent/10 text-accent text-[11px] font-bold px-2 py-0.5 rounded border border-[#24b47e]/20 uppercase">{item.notes}</span>
                    </div>
                    <div className="text-muted text-sm font-medium">Count: {item.qty || 1}</div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {item.options?.map((opt, i) => {
                      ...
                      return (
                        <div key={i} onClick={() => selectPartOption(idx, i)} className={`flex items-center p-4 rounded-xl border cursor-pointer transition-all ${isSelected ? 'border-[#24b47e] bg-accent/5' : 'border-default bg-white dark:bg-surface-strong hover:border-neutral-600'}`}>
                          ...
                          <div className="flex-1">
                            <div className="text-primary font-bold text-[15px] mb-1">{item.local} ({opt.type})</div>
                            <div className="text-muted text-[13px] flex gap-6">
                          ...
                          <div className="text-accent font-bold text-sm">{opt.match} match</div>
                        </div>
                      );
                    })}
                  </div>
            ...
            <button onClick={() => setStep(1)} className="mt-8 text-muted hover:text-primary transition-colors flex items-center text-sm font-semibold">
        ...
        {step === 3 && currentProject && (
          <motion.div key="step3" variants={slideVariants} initial="initial" animate="enter" exit="exit" className="w-full">
            <div className="flex items-start justify-between mb-8">
              ...
              <button onClick={() => setStep(4)} className="bg-accent hover:bg-accent-strong text-black font-bold rounded-lg px-6 py-2.5 active:scale-[0.98] transition-all flex items-center shadow-[0_0_15px_rgba(36,180,126,0.2)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div 
                ...
                className={`bg-[#f8f9fa] dark:bg-surface-soft border rounded-2xl p-8 cursor-pointer transition-all ${!useSmartRecommendations ? 'border-[#3ecf8e] shadow-[0_0_20px_rgba(36,180,126,0.1)]' : 'border-default hover:border-neutral-600'}`}
              >
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-primary font-bold text-lg">Standard Selections</h3>
                  {!useSmartRecommendations && <div className="bg-accent text-black text-xs font-bold px-2 py-1 rounded">SELECTED</div>}
                </div>
                <div className="text-4xl font-black text-primary mb-6">₱{calculateTotalCost(currentProject).toLocaleString(undefined, {maximumFractionDigits: 0})}</div>
                <ul className="space-y-4 text-muted text-sm">
                  <li className="flex items-start"><span className="text-muted mr-2">•</span> Uses your exact manual selections</li>
                  <li className="flex items-start"><span className="text-muted mr-2">•</span> Multiple individual shipping fees</li>
                  <li className="flex items-start"><span className="text-muted mr-2">•</span> Standard retail pricing</li>
                </ul>
              </div>
              ...
              >
                ...
                <ul className="space-y-5 text-[13px]">
                  <li className="flex items-center"><span className="bg-accent/20 text-accent font-bold px-2 py-0.5 rounded border border-[#24b47e]/30 mr-4 shrink-0 uppercase">REPLACED</span> <span className="text-neutral-300">In-stock equivalent board suggestion (Fits project specs)</span></li>
                  <li className="flex items-center"><span className="bg-accent/20 text-accent font-bold px-2 py-0.5 rounded border border-[#24b47e]/30 mr-4 shrink-0 uppercase">CONSOLIDATED</span> <span className="text-neutral-300">Shipped from single supplier (Saved separate shipping fees)</span></li>
                  <li className="flex items-center"><span className="bg-accent/20 text-accent font-bold px-2 py-0.5 rounded border border-[#24b47e]/30 mr-4 shrink-0 uppercase">BULK DEALS</span> <span className="text-neutral-300">Swapped to multi-pack cables (Saves unit cost)</span></li>
                </ul>
              </div>
            </div>

            <button onClick={() => setStep(2)} className="mt-8 text-muted hover:text-primary transition-colors flex items-center text-sm font-semibold">
        ...
        {step === 4 && currentProject && (
          <motion.div key="step4" variants={slideVariants} initial="initial" animate="enter" exit="exit" className="w-full" id="maker-results-dashboard">
            <div className="mb-8">
              <h1 className="text-[32px] font-extrabold text-primary tracking-tight">
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex-1 bg-[#f8f9fa] dark:bg-surface-soft border border-default rounded-2xl overflow-hidden flex flex-col">
                <div className="p-6 border-b border-default bg-surface-strong">
                  <h3 className="text-primary font-bold text-[17px]">Shopping Cart Items</h3>
                </div>
                  ...
                  {currentProject.components?.map((item, idx) => {
                    ...
                    return (
                      <div key={idx} className="p-6 flex items-center justify-between hover:bg-white/[0.02] transition-colors">
                        <div>
                          <div className="text-primary font-bold text-[15px] mb-1">{item.local}</div>
                          <div className="text-muted text-[13px]">{item.notes} (Qty: {item.qty || 1})</div>
                        </div>
                        <div className="text-primary font-bold text-right">
                           ...
                           {useSmartRecommendations && <div className="text-accent">₱{(price * (item.qty || 1) * 0.75).toLocaleString(undefined, {maximumFractionDigits: 0})}</div>}
                        </div>
                      </div>
                    );
                  })}
                ...
                {isUpdatingMode && (
                  <div className="mt-auto p-6 border-t border-default bg-surface-strong flex justify-start">
                    <MarkCompleteCard 
              ...
              <div className="w-full lg:w-[420px] flex flex-col space-y-6">
                ...
                <div className="bg-[#f8f9fa] dark:bg-surface-soft border border-default rounded-2xl p-4 shadow-xl">
                   <h3 className="text-primary font-bold text-[15px] mb-3 flex items-center"><FiMapPin className="text-accent mr-2" /> Supply Chain Map</h3>
                   <div className="w-full h-[300px] rounded-xl overflow-hidden border border-default">
                     <StoreMap />
                   </div>
                </div>

                <div className="bg-[#f8f9fa] dark:bg-surface-soft border border-default rounded-2xl p-8 shadow-xl">
                  <h3 className="text-primary font-bold text-[18px] mb-6">Order Summary</h3>
                  
                  <div className="flex justify-between mb-5 border-b border-default pb-5">
                    <span className="text-muted">Final Total Cost</span>
                    <span className="text-primary font-black text-xl">₱{(useSmartRecommendations ? calculateTotalCost(currentProject) * 0.75 : calculateTotalCost(currentProject)).toLocaleString(undefined, {maximumFractionDigits: 0})}</span>
                  </div>
                  
                  <div className="flex justify-between mb-8">
                    <span className="text-muted">Sourcing Status</span>
                    <span className="text-accent font-semibold">All items ready</span>
                  </div>

                  <div className="space-y-4">
                    <button onClick={saveCartToDatabase} className="w-full bg-accent hover:bg-accent-strong text-black font-bold rounded-xl px-6 py-4 active:scale-[0.98] transition-all flex items-center justify-center shadow-[0_0_20px_rgba(36,180,126,0.2)] text-[13px] tracking-wide uppercase">
                    ...
                    <div className="relative">
                      <button 
                        ...
                        className="w-full bg-transparent border border-[#24b47e]/50 hover:border-[#3ecf8e] text-accent font-bold rounded-xl px-6 py-4 active:scale-[0.98] transition-all flex items-center justify-center text-[13px] tracking-wide"
                      >
                      ...
                      <AnimatePresence>
                        {showExportMenu && (
                          <motion.div 
                            ...
                            exit={{ opacity: 0, y: 10 }}
                            className="absolute bottom-full left-0 right-0 mb-3 bg-white dark:bg-surface-strong border border-[#3ecf8e]/30 rounded-xl shadow-2xl overflow-hidden flex flex-col z-10"
                          >
                            <button onClick={handleDownloadWord} className="flex items-center px-5 py-4 text-primary hover:bg-panel-strong hover:text-accent transition-colors border-b border-default text-[13px] font-semibold text-left">
                              <FiFileText className="mr-3 w-5 h-5" /> Download as Word Document
                            </button>
                            <button onClick={handleDownloadExcel} className="flex items-center px-5 py-4 text-primary hover:bg-panel-strong hover:text-accent transition-colors border-b border-default text-[13px] font-semibold text-left">
                              <FiGrid className="mr-3 w-5 h-5" /> Export as Excel (CSV)
                            </button>
                            <button onClick={handleDownloadPDF} id="maker-download-btn" className="flex items-center px-5 py-4 text-primary hover:bg-panel-strong hover:text-accent transition-colors border-b border-default text-[13px] font-semibold text-left">
                              <FiFile className="mr-3 w-5 h-5" /> Generate PDF Report
                            </button>
                            <button onClick={handleDownloadImage} className="flex items-center px-5 py-4 text-primary hover:bg-panel-strong hover:text-accent transition-colors text-[13px] font-semibold text-left">
            ...
            <button onClick={() => setStep(3)} className="mt-8 text-muted hover:text-primary transition-colors flex items-center text-sm font-semibold">
        ...
        {viewProject && (
          ...
          >
            <motion.div 
              ...
              className="bg-[#f8f9fa] dark:bg-surface-soft border border-default rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl"
            >
              <div className="p-6 border-b border-default flex justify-between items-center bg-white dark:bg-surface-strong">
                <div>
                  <h3 className="text-primary font-bold text-xl">{viewProject.title}</h3>
                  <p className="text-accent text-sm mt-1 font-semibold">Total Budget: {viewProject.cost}</p>
                </div>
                <button onClick={() => setViewProject(null)} className="text-muted hover:text-primary transition-colors bg-neutral-100 dark:bg-neutral-900 hover:bg-neutral-200 dark:bg-neutral-800 p-2 rounded-lg">
                  <FiX className="w-5 h-5" />
                </button>
              </div>
              
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <h4 className="text-primary font-bold text-sm mb-4 uppercase tracking-wider text-muted">Project Parts List</h4>
                <div className="space-y-3">
                  {viewProject.components?.map((comp, idx) => (
                    <div key={idx} className="bg-soft border border-default rounded-xl p-4 flex items-center justify-between">
                      <div>
                        <div className="text-primary font-bold text-[15px] mb-1">{comp.local}</div>
                        <div className="text-muted text-xs">Original: {comp.orig}</div>
                      </div>
                      <div className="text-primary font-bold text-sm bg-white dark:bg-black border border-default rounded-lg px-3 py-1">
                        Qty: {comp.qty || 1}
                      </div>
                    </div>
                  ))}
              ...
              <div className="p-6 border-t border-default bg-white dark:bg-surface-strong flex justify-end">
                <button 
                  onClick={() => {
                  ...
                  className="bg-accent hover:bg-accent-strong text-black font-bold rounded-lg px-6 py-2.5 transition-colors text-sm flex items-center"
                >
